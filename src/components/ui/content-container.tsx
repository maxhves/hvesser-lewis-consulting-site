...
export function ContentContainerNew({
  ...
  return (
    <div className={clsx(outerClassName, 'relative flex justify-center px-6 md:px-8')}>
      <div className="absolute inset-y-0 left-1/2 z-0 w-px bg-stone-200 dark:bg-stone-800"></div>
      <div
        className={clsx(
          className,
          'z-10 flex w-full max-w-6xl flex-col border-x border-stone-200 dark:border-stone-800'
        )}
      >
        {children}
      </div>
